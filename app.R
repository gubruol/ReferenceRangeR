...
rrr_theme <- bs_theme(version = 5, preset = "shiny", primary = "#003B73",
...
UMO_SVG <- local({
  ...
  HTML(gsub("#ffffff", "#003B73", s, ignore.case = TRUE))
})
...
rrr_head <- tagList(
  ...
  tags$style(HTML("
    :root {
      --rrr-page: #F4F6FA; --rrr-surface: #FFFFFF; --rrr-line: rgba(16,24,40,.09);
      --rrr-brand: #003B73; --bs-body-bg: #F4F6FA;
    ...
    .navbar { background: var(--rrr-surface) !important;
    ...
    .navbar-brand img { width: 40px; height: 40px; border-radius: 50%;
                        ...
                        padding: 3px; background: var(--rrr-brand); box-sizing: border-box; }
    /* The navbar never collapses (navbar_options(collapsible = FALSE) drops the
       toggler). Without a .navbar-expand-* class Bootstrap would still stack it
       vertically, so reproduce the expanded layout at every width: the tabs stay
       on one row, and only the outer row wraps if the window gets very narrow. */
    .navbar { justify-content: flex-start; }
    .navbar > .container-fluid, .navbar > .container { align-items: center;
                                                       flex-wrap: wrap; row-gap: .25rem; }
    .navbar .navbar-nav { align-items: center; flex-direction: row; flex-wrap: nowrap; }
    ...
    .navbar .nav-underline :where(ul.nav.navbar-nav > li) > .nav-link {
      padding-top: .25rem; padding-bottom: .25rem; margin-bottom: 0; }
    ...
    .rrr-umo svg { height: 54px; width: auto; display: block; }
    ...
    .rrr-tile { background: var(--rrr-surface) !important; color: var(--bs-body-color) !important;
                border: 1px solid var(--rrr-line) !important; }
    .rrr-tile .value-box-title { font-weight: 500; opacity: .75; }
    .rrr-tile .value-box-value { font-weight: 640; letter-spacing: -0.02em; }
    ...
    .card, .card-body, .bslib-sidebar-layout > .main {
...
sex_radio <- function(id) radioButtons(id, "Sex", inline = TRUE,
...
ui <- page_navbar(
  ...
  nav_panel("Input Data", value = "data",
    layout_columns(col_widths = c(5, 7), fill = FALSE,
      navset_card_underline(
        ...
        nav_panel("Paste Data from Clipboard", div(class = "pt-2",
          textAreaInput("pasted", NULL, rows = 5, width = "100%",
                        placeholder = "e.g.\nresult  age  sex\n5.4  42  F\n6.1  37  M"),
          div(class = "form-text mb-2", "Paste with a header line, e.g. copied from Excel."),
          actionButton("use_paste", "Use pasted data", class = "btn-primary w-100"))),
        nav_panel("Demo", div(class = "pt-2",
          p(class = "mb-3", "50,000 simulated results with an age-dependent median ",
            "and about 10% pathological values."),
          actionButton("use_demo", "Generate demo data", class = "btn-primary w-100")))),
      card(card_header("Columns"), uiOutput("column_ui"), uiOutput("sex_label_ui"))),
  ...
  nav_panel("Check Data", value = "explore",
    layout_sidebar(
      ...
      sidebar = sidebar(width = 290, open = "always", resizable = FALSE,
        sex_radio("ex_sex"), age_range_input("ex_age_from", "ex_age_to"),
        conditionalPanel("input.advanced",
          sliderInput("max_groups", "Maximum number of age groups",
                      min = 2, max = 12, value = 3, step = 1, ticks = FALSE)),
        task_button("run_sex", "Check sex differences", class = "btn-primary w-100 mt-2"),
        task_button("run_drift", "Check age drift", class = "btn-primary w-100 mt-2")),
    ...
    layout_sidebar(
      ...
      sidebar = sidebar(width = 290, open = "always", resizable = FALSE,
        ...
        conditionalPanel("output.has_trimester && input.advanced",
          radioButtons("ri_trimester", "Trimester", inline = TRUE,
        ...
        checkboxInput("compare", "Compare with known limits", value = FALSE),
  ...
  nav_menu(title = icon("gear"), align = "right",
    nav_item(input_switch("advanced", "Advanced", value = FALSE)),
...
server <- function(input, output, session) {
  ...
  output$column_ui <- renderUI({
    df <- raw()
    if (is.null(df)) return(hint("Load a file, paste data or generate demo data to continue."))
    g <- guessed(); cols <- names(df)
    optional <- function(id, label, sel)
      selectInput(id, label, choices = c("(none)" = "", cols), selected = sel %||% "",
                  width = "100%")
    layout_columns(col_widths = c(6, 6, 6, 6), fill = FALSE,
      selectInput("col_result", "Result", choices = cols, selected = g$result,
                  width = "100%"),
      optional("col_age", "Age", g$age), optional("col_sex", "Sex", g$sex),
      conditionalPanel("input.advanced",
                       optional("col_trimester", "Trimester", g$trimester)))
  })
  ...
  output$sex_label_ui <- renderUI({
    ...
    tagList(hr(),
      ...
      layout_columns(col_widths = c(6, 6), fill = FALSE,
        selectInput("lab_female", "female", choices = choices, multiple = TRUE,
                    selected = picked("lab_female")),
        selectInput("lab_male", "male", choices = choices, multiple = TRUE,
  ...
  output$data_summary <- renderUI({
    ...
    tagList(
      do.call(layout_column_wrap,
              c(list(width = "220px", fill = FALSE, class = "mb-3"),
  ...
  output$preview_controls <- renderUI({
    ...
    div(class = "d-flex align-items-center justify-content-between gap-2 mb-2",
        div(class = "form-text mt-0", if (original)
  ...
  output$explore_summary <- renderUI({
    r <- explore_res()
    early <- result_or(r, paste(
      "Two checks are available: whether the results differ between the sexes,",
  ...
  sex_summary_ui <- function(v) {
    ...
    verdict <- if (v$n_groups < 2 || !isTRUE(is.finite(v$p_value)))
      hint("More than one group is needed for a statistical comparison.")
  ...
  drift_summary_ui <- function(v) tagList(
    p(class = "small mb-2", sprintf("n = %s selected", big(v$n)),
      if (isTRUE(v$subsampled))
        span(br(), sprintf("The curve was fitted on a random subsample of %s.", big(v$n_used)))),
    if (!v$stratification_needed)
      div(class = "text-success fw-semibold", "No stratification by age necessary.")
    else tagList(
      div(class = "fw-semibold mb-2", "Age groups with comparable medians"),
  ...
  output$ri_summary <- renderUI({
    ...
    tagList(html_dl(rows), hr(class = "my-2"),
      ...
      if (!is.null(s$comparison)) p(class = "small text-secondary mb-1",
        sprintf("Comparison limits: %s - %s",
                format_limit(s$comparison[1]), format_limit(s$comparison[2]))),
      if (isTRUE(is.finite(v$lambda))) p(class = "small text-secondary mb-0",
        paste0(sprintf("Estimated \u03bb: %.2f", v$lambda),
               if (nzchar(v$lambda_note %||% "")) paste0(" ", v$lambda_note))),
      if (nzchar(v$note)) p(class = "small text-secondary mb-0", v$note))
  })
