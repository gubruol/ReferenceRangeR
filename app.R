...
server <- function(input, output, session) {
    ...
    if (casesfiltered < 500) shinyalert("insufficient data...", paste("The selected dataset contains ", casesfiltered, " results.\nPlease increase the sample size (minimum n=500)."), type = "error")
      ...
      output$strat_placeholder = renderText({
        paste("<br>",
              ...
              if (pvalue >= 0.0 && pvalue < 0.05 && n_groups == 2) "<i style='font-size: 10px;'>For large samples, the Wilcoxon test may yield significance for negligible effects. Consider using a stricter alpha-level to ensure practical relevance.</i>",
  ...
  observeEvent(input$drift, {
    ...
    if(!"mgcv" %in% .packages()) library(mgcv)
    if(!"qgam" %in% .packages()) library(qgam)
